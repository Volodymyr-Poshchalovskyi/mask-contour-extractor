from PyQt6.QtWidgets import QWidget, QHBoxLayout, QCheckBox, QLineEdit, QPushButton, QColorDialog
...
class ObjectListItem(QWidget):
    ...
    def init_ui(self):
        ...
        # Візуалізація відсутності об'єкта
        if not self.obj_data.is_present_in_frame:
            self.le_name.setStyleSheet("color: #888; font-style: italic;")
            self.le_name.setToolTip("Цього об'єкта немає на поточному кадрі")
    ...
    def on_visibility_change(self):
        is_checked = self.cb_visible.isChecked()
        self.app.sync_visibility(self.obj_data.display_name, is_checked)
